...
import pandas as pd
import numpy as np
import Hybrid_f2_SEB, Hybrid_main2_SEB
...
EC_total['VPD_Avg'] = Hybrid_f2_SEB.vpd_calc(EC_total['TA_Avg'].values,EC_total['RH_Avg'].values)/100 #from pa to hpa

EC_total['ra'] = Hybrid_f2_SEB.ra_SEB_Inv(EC_total['TA_Avg'].values,
                                      EC_total['P_Avg'].values,
                                      EC_total['TS_Avg'].values,
                                      EC_total['Rn_Avg'].values,
                                      EC_total['G_Avg'].values,
                                      EC_total['LE'].values)
EC_total.loc[(EC_total['ra']<0)|(EC_total['ra']>2000),'ra'] = np.nan
EC_total = EC_total[(EC_total['ra'] >= EC_total['ra'].quantile(0.05)) & (EC_total['ra'] <= EC_total['ra'].quantile(0.95))]

'''检查1 True'''
# EC_total['LE_calc'] = Hybrid_main2_SEB.ETa(EC_total['TA_Avg'].values,
#                                        EC_total['P_Avg'].values,
#                                        EC_total['TS_Avg'].values,
#                                        EC_total['Rn_Avg'].values,
#                                        EC_total['G_Avg'].values,
#                                        EC_total['ra'].values)
# EC = EC_total[['LE_Corr','LE_calc']]
...
# #对特征进行Z-score标准化
# X2_shuffle_scaled = pd.DataFrame(scaler.fit_transform(X2_shuffle), columns=X2_shuffle.columns)
